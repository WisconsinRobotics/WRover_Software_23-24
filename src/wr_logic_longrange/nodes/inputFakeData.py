    ...
    for t in range(180):
        if t >= angle1 and t <= angle2:
            if t != 0:
                ...
                y = (obstacle.coord1[1] - obstacle.coord1[0] * m_slope) / (
                    1 - (m_slope / m_angle)
                )
                x = y / m_angle
...
def updateHeading(data) -> None:
    ...
    maxVelocity = 0.8  ### in meters per second
    ...
    # -----------------
    laser_adjust = (data.left_value * maxVelocity - data.right_value * maxVelocity) / (
        ROBOT_WIDTH / 2
    )
    laser_adjust = laser_adjust * (1 / RATE)
    # rospy.logerr(laser_adjust)

    mock_heading = mock_heading + math.degrees(laser_adjust)
    ...
    obstacle.setCoord1(
        obstacle.coord1[0] * math.cos((angle_adjust))
        + obstacle.coord1[1] * math.sin((angle_adjust)),
        obstacle.coord1[1] * math.cos((angle_adjust))
        - obstacle.coord1[0] * math.sin((angle_adjust)),
    )

    obstacle.setCoord2(
        obstacle.coord2[0] * math.cos((angle_adjust))
        + obstacle.coord2[1] * math.sin((angle_adjust)),
        obstacle.coord2[1] * math.cos((angle_adjust))
        - obstacle.coord2[0] * math.sin((angle_adjust)),
    )

    # rospy.logerr(str(obstacle.coord2[0]) + " " + str(obstacle.coord2[1]))

    # ******* Reset obstacle when behind us *************#
    if obstacle.coord1[1] < 0.1 and obstacle.coord2[1] < 0.1:
        obstacle.testAngle()
