...
# TODO(@bennowotny): Review/select better sensor, or fix current thermal problems
...
class BNO055:
    ...
    def begin(self, mode=None):
        ...
        self.writeBytes(BNO055.BNO055_PWR_MODE_ADDR,
                        [BNO055.POWER_MODE_NORMAL])
        time.sleep(0.01)
    ...
    def setExternalCrystalUse(self, useExternalCrystal=True):
        ...
        time.sleep(0.025)
        self.writeBytes(BNO055.BNO055_PAGE_ID_ADDR, [0])
        self.writeBytes(BNO055.BNO055_SYS_TRIGGER_ADDR, [
                        0x80] if useExternalCrystal else [0])
        time.sleep(0.01)
    ...
    def getQuat(self):
        ...
        wxyz = (struct.unpack('hhhh', ''.join(struct.pack(
            'BBBBBBBB', buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], buf[6], buf[7]))))
