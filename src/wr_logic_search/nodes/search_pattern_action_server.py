...
class SearchActionServer:
    def __init__(self, name) -> None:
        ...
        """
        self.long_range_client = actionlib.SimpleActionClient(
            "LongrangeActionServer",
            LongRangeAction
        )

        self.spin_client = actionlib.SimpleActionClient(
            "SpinActionServer",
            SpinAction
        )
    ...
    def execute_callback(self, goal: SearchStateGoal): 
        ...
        # NOTE: in the future, please set timeouts using the action client, not the server
        ...
        while (
            ...
            self.long_range_client.send_goal(LongRangeGoal(coords[i]['lat'], coords[i]['long']))
            self.long_range_client.wait_for_result(rospy.Duration(timeout))

            if self.long_range_client.get_state() == actionlib.GoalStatus.SUCCEEDED:
                # spin to search for vision target
                self.spin_client.send_goal(SpinGoal())
                self.spin_client.wait_for_result()
                object_detected = self.spin_client.get_state() == actionlib.GoalStatus.SUCCEEDED
                if object_detected:
                    break
